# Zenlytic UI

This section covers all aspects of the Zenlytic user interface and its features.

## Contents

- [Using Zenlytic](using_zenlytic.md)
- [Zoe](zoe.md) 
- [Exploring](exploring.md)
- [Data Model Editor](data_model_editor.md)
- [User Attributes](user_attributes.md)
- [User Roles](user_roles.md)
- [Workspace Groups and Permissions](workspace_groups_and_permissions.md)
- [Follow Ups](follow_ups.md)
- [Following Up From A Dashboard](following_up_from_a_dashboard.md)
- [Microsoft Teams Bot](microsoft_teams_bot.md)