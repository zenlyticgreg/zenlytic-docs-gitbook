...
Access control is managed in Zenlytic through two concepts. The first, [access grants](../5_data_modeling/access_grants.md#access-grants), cover *column-based* access control, and the second, [access filters](../5_data_modeling/access_grants.md#access-filters), cover *row-based* access control.

## Access grants

Access grants are restrictions for certain users on the ability to see various fields and query them in the Zenlytic interface. These access restrictions are based on access to *columns* and *views*. For row-based access control look at [access filters](../5_data_modeling/access_grants.md#access-filters).
...
Access filters are restrictions for certain users on the ability to see various rows and query them in the Zenlytic interface. These access filters protect data access based on access to *rows* in a view. For column-based access control look at [access grants](../5_data_modeling/access_grants.md#access-grants).

They are specified in [view](../5_data_modeling/view.md) files and apply to all queries that reference that view.
...
User attributes are strings that can handle [filter syntax](../5_data_modeling/field_filter.md) for specifying complex comparisons or inclusions in either access grants (column level security) or access filters (row level security).
